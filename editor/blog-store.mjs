...
function validatePost(post) {
  ...
  if (!CATEGORIES.has(clean.category)) throw new Error('Type must be Technical or Personal.');
...
async function readMeta(slug, html = '') {
  try {
    ...
    const { tags: _legacyTags, ...cleanMeta } = meta;
    return { ...cleanMeta, slug, category };
  }
  ...
  return { slug, title, description, author: meta[0] || 'Bryson', date: '2026-09-01', category: 'technical' };
}

function taxonomyMarkup(post) {
  return `<div class="post-taxonomy"><span class="type-badge type-${escapeHtml(post.category)}">${CATEGORY_LABELS[post.category]}</span></div>`;
}
...
async function renderPostHtml(post) {
  ...
  if (!existing.includes('.post-taxonomy{')) {
    const taxonomyCss = `.post-taxonomy{display:flex;margin:-24px 0 38px}.type-badge{display:inline-flex;padding:4px 9px;border-radius:999px;font-size:.72rem;font-weight:800;letter-spacing:.04em;color:var(--white);background:var(--green);text-transform:uppercase}\n`;
...
function blogIndex(posts) {
  const categories = ['technical', 'personal'];
  ...
  const items = posts.map((post) => {
    return `<article class="post-item" data-post data-category="${post.category}">
...
<main><p class="eyebrow">Writing</p><h1>Blogs about my projects and personal life.</h1><p class="intro">Technical builds, personal essays, and notes from me.</p><p class="human-note">Written by me, not AI.</p>
<section class="filters" aria-label="Filter articles"><div class="filter-row"><span class="filter-label">Type</span><button type="button" class="active" data-category="all">All</button>${typeButtons}</div></section>
...
<script>(()=>{let category='all';const posts=[...document.querySelectorAll('[data-post]')],empty=document.querySelector('.empty');function apply(){let visible=0;for(const post of posts){const show=category==='all'||post.dataset.category===category;post.hidden=!show;if(show)visible++}empty.hidden=visible>0;const params=new URLSearchParams;if(category!=='all')params.set('type',category);history.replaceState(null,'',location.pathname+(params.size?'?'+params:''))}document.addEventListener('click',event=>{const button=event.target.closest('[data-category]');if(!button)return;category=button.dataset.category;document.querySelectorAll('[data-category]').forEach(item=>item.classList.toggle('active',item.dataset.category===category));apply()});category=new URLSearchParams(location.search).get('type')||'all';document.querySelectorAll('[data-category]').forEach(item=>item.classList.toggle('active',item.dataset.category===category));apply()})()</script></body></html>\n`;
}
...
export async function savePost(input, originalSlug = '') {
  ...
  await Promise.all([
    ...
    fs.writeFile(path.join(dir, 'meta.json'), JSON.stringify({ slug: post.slug, title: post.title, description: post.description, author: post.author, date: post.date, category: post.category }, null, 2) + '\n'),
